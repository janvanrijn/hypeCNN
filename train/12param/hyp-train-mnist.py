...
def get_hyperparameter_search_space(seed=None):
    ...
    tolerance = ConfigSpace.UniformFloatHyperparameter(
        name='tolerance', lower=0.00001, upper=0.001, log=True, default_value=0.0001)
...
def load_data(shuffle, batch_size, resize_crop, h_flip, v_flip):
    ...
    t_list += [transforms.ToTensor(), transforms.Normalize((0.1307,), (0.3081,)), transforms.Lambda(lambda x: x.repeat(3, 1, 1) ),]
