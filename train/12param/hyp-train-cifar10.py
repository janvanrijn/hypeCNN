...
def load_data(shuffle, batch_size, resize_crop, h_flip, v_flip):
    root_dir = '/rigel/dsi/users/as5414/cifar10/data/'
    t_list = []
    if resize_crop:
    ...
    trainset = datasets.CIFAR10(root=root_dir, train=True, download=True, transform=transform_train)
    train_loader = torch.utils.data.DataLoader(trainset, batch_size=batch_size, shuffle=shuffle)
    testset = datasets.CIFAR10(root=root_dir, train=False, download=True, transform=transform_test)
