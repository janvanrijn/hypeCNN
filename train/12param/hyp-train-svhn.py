...
def load_data(shuffle, batch_size, resize_crop, h_flip, v_flip):
    root_dir = '/rigel/dsi/users/as5414/svhn-search/data/'
    t_list = []
    if resize_crop:
    ...
    trainset = datasets.SVHN(root=root_dir, split='train', download=True, transform=transform_train)
    train_loader = torch.utils.data.DataLoader(trainset, batch_size=batch_size, shuffle=shuffle)
    testset = datasets.SVHN(root=root_dir, split='test', download=True, transform=transform_test)
